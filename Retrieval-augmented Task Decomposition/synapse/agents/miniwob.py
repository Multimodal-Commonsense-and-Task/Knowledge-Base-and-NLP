...
TMP=False # NOTE: set to True to use miniwob++ from CompWob
...
from synapse.utils.compwob import get_subtasks
import ipdb
    ...
    def reset(self, seed: int) -> None:
        ...
        if self.args.no_memory:
            ...
            if self.args.env_name.startswith("compositional."): # a CompWob task
                subtasks = get_subtasks(self.task)
                first_task = subtasks[0]
                second_task = subtasks[1]

                if self.args.compwob_prompting_strategy == 'first':
                    top_k_exemplars_first = retrieve_exemplar_name(self.memory, query, 3)
                    ipdb.set_trace()
