...
def eval_traj_sample(task_id, args, sample):
    ...
    for s, act_repr in zip(sample["actions"], sample["action_reprs"]):
        ...
        def narrate(exemplar):
            """
            Message to narrate an exemplar
            
            """
            #print(f"Narrating exemplar")
            start_time = time.time()
            narrate_sys_message = [
                                {
                                    "role": "system",
                                    "content": "You are a large language model trained to navigate the web. You will be given demonstrations of a how to navigate a web UI to perform tasks. Your goal will be to summarize these demonstrations."
                                }
                            ]
            
            narrate_message = narrate_sys_message + exemplar

            narrate_query = copy.deepcopy(query)
            narrate_query[-1]["content"] = f"""Summarize the above sequence of observations and actions demonstrated, explaining the purpose and effect of each action with respect to the task."""
            narrate_message = narrate_message + narrate_query
            narrate_response, narrate_info = generate_response(messages=narrate_message, model=model, temperature=args.temperature, stop_tokens=[],   )
            #ipdb.set_trace()

            #print(f"Narration of exemplar: {narrate_response}")
            narrate_demo_message = []
            for line in narrate_response.split("\n"):
                if not line == "":
                    narrate_demo_message.append({"role": "user", "content": line})      
            end_time = time.time()
            #print(f"Narration time: {end_time - start_time}")                  
            return narrate_demo_message
        ...
        #ipdb.set_trace()
        def lookahead1(demo_message, query):
            """
            Message to narrate an exemplar
            ...
            #ipdb.set_trace()
            lookahead_query = copy.deepcopy(query)
            #lookahead_query[-1]["content"] = f"""Evaluate each of the elements with respect to the task. For each element, output the action that you would take next, and the expected outcome of that action, based on the demonstrations."""
            lookahead_query[-1]["content"] = f"""For all elements with ids ({ids}), evaluate the possible element-action combinations with respect to the task. 
            Use the following format:             
            - id of the element, 
            - description of the element, if any.
            - for each possible action operation that can be done using that element, the expected outcome of that action, based on the demonstrations."""
            lookahead_message = sys_message + demo_message + query + lookahead_query           
            lookahead_response, lookahead_info = generate_response(messages=lookahead_message, model=model, temperature=args.temperature, stop_tokens=[])
            #ipdb.set_trace()
        ...
        def lookahead(demo_message, query):
            """
            Message to narrate an exemplar
            ...
            # nar2lookahead3
            #lookahead_query[-1]["content"] = f"""For each element id in ({ids}), or each element, output the action that you would take next, and the expected outcome of that action, based on the demonstrations."""

            # plan + narrate
            lookahead_query[-1]["content"] = f"""Think step-by-step. First, give a concise plan of the remaining steps to complete the task. Make sure you only output the remaining steps, and not the steps that have already been completed. Based on this, given type of the next action, evaluate the elements with ids ({ids}), for expected outcome of the action on each element."""
            
            # nar2lookahead4
            #lookahead_query[-1]["content"] = f"""Think step-by-step. Give a concise plan of the remaining steps to complete the task. Make sure you only output the remaining steps, and not the steps that have already been completed. Here are the actions taken so far:\n{actions_str}"""
        ...
        """
        lookahead_response = lookahead(demo_message, query)
        #ipdb.set_trace()

        #query[-1]["content"] = f"Here are the evaluations of the choices, based on the demonstrations:\n" + lookahead_response + '\n' + query[-1]["content"]

        #query[-1]["content"] += f"\nEvaluations of the choices, based on the demonstrations:\n" + lookahead_response
        lookahead_message = {"role": "user", "content": f"\nEvaluations of the choices, based on the demonstrations:\n" + lookahead_response}
        tmp_query = copy.deepcopy(query)
        if len(tmp_query) == 1:
            #ipdb.set_trace()
            #tmp_query = [lookahead_message] + tmp_query
            pass
        else:
            #ipdb.set_trace()
            tmp_query.insert(-1, lookahead_message)
        message = sys_message + demo_message + tmp_query
        """
