...
def eval_traj_sample(task_id, args, sample):
    ...
    for ii, (s, act_repr) in enumerate(zip(sample["actions"], sample["action_reprs"])):
        ...
        #ipdb.set_trace()
        try:
            if ii+1 == len(sample["actions"]):
                pass
            else:
                tmp1 = etree.fromstring(sample["actions"][ii]['cleaned_html'])
                tmp2 = etree.fromstring(sample["actions"][ii+1]['cleaned_html'])
                diffs = compare_elements(tmp1, tmp2)
                print(f"n diffs: {len(diffs)}")
                ...
                if len(diffs) < 100:
                    ...
                    for a in sample["action_reprs"]:
                        print(a)
                    print(f"\n### Current action: {sample['action_reprs'][ii]}")
                    print(f"### (Next action): {sample['action_reprs'][ii+1]}")
                    print(f"\n### Diffs before and after Current action:")
                    for d in diffs:
                        print(d)
                    ipdb.set_trace()

        except IndexError:
            print(f"IndexError: {ii}")
            ipdb.set_trace()
