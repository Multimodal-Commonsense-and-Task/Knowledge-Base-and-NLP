...
def eval_traj_sample(task_id, args, sample):
    ...
    for s, act_repr in zip(sample["actions"], sample["action_reprs"]):
        ...
        def annotate(demo_message):
            ...
            annotate_llm = 'gpt-4-0613'
            #annotate_llm = 'gpt-4-32k-0613'            
            #annotate_llm = 'gpt-4-32k'
            #annotate_llm = 'gpt-4-1106-preview'
            #annotate_llm = 'gpt-3.5-turbo-16k'
            #annotate_llm = 'gpt-3.5-turbo-16k-0613'
            #annotate_llm = 'gpt-3.5-turbo-1106'
            
            set_api('openai', annotate_llm)
            lookahead_response, lookahead_info = generate_response(messages=lookahead_message, model=annotate_llm, temperature=args.temperature, stop_tokens=['EOS'])
            #ipdb.set_trace()
            ...
            for item in original_trajectory:
                if item['role'] == 'user' and item['content'].startswith('act:'):
                    ...
                    try:
                        ...
                        #ipdb.set_trace()
                        print(f"GPT output was too short")
                    
                    if '\nAct' in thought_content:
                        target_str = thought_content.split('\nAct')[0]
                        #ipdb.set_trace()
                    elif '\nact' in thought_content:
                        target_str = thought_content.split('\nact')[0]
                        #ipdb.set_trace()
                    else:
                        # If there is no act, then just use the whole thought
                        target_str = thought_content

                    thought_content2 = target_str
                    #thought_content2 = thought_content.split('\nact')[0] + '\n' + item['content']

                    new_trajectory.append({'role': 'user', 'content': thought_content2})
        ...
        for e_id, e in enumerate(exemplars):
            ...
            if exemplar_id not in exemplars_annotations:
                ...
                tries = 0
                while not annotate_success:
                    try:
                        e_thoughts = annotate(e)
                        annotate_success = True
                    except Exception as e:
                        print(f"An error occurred: {e}")
                        print(f"Doing try {tries}")
                        tries += 1
                        if tries > 5:
