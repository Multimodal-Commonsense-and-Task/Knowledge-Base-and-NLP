...
def eval_traj_sample(task_id, args, sample):
    ...
    permutation_detection = Counter()
    permutation_detection['n_total'] = 0
    permutation_detection['n_permutable'] = 0
    ...
    for ii, (s, act_repr) in enumerate(zip(sample["actions"], sample["action_reprs"])):
        permutation_detection['n_total'] += 1
        ...
        try:
            ...
            for jj, (_s, _act_repr) in enumerate(zip(sample["actions"], sample["action_reprs"])):
                ...
                if jj != ii:
                    ...
                    # only check jj > ii, that is we will see if an action that was originally taken later in the trajectory
                    # can be taken earlier in the trajectory, and lead up to the state before action ii
                    if jj <= ii:
                        continue
                    if use_clean:
                        tmp1 = etree.fromstring(sample["actions"][ii]['cleaned_html']) # before taking action ii
                        tmp2 = etree.fromstring(sample["actions"][jj+1]['cleaned_html']) # after taking action jj
                    else:
                        tmp1 = etree.fromstring(sample["actions"][ii]['raw_html']) # before taking action ii
                        tmp2 = etree.fromstring(sample["actions"][jj+1]['raw_html']) # after taking action jj
                    ...
                    if len(diffs) < 10:
                        ...
                        if flag:
                            ...
                            for a in sample["action_reprs"]:
                                print(a)
                            print(f"\n### HTML BEFORE Current action #{ii}: {sample['action_reprs'][ii]}")
                            print(f"### Compared with AFTER action #{jj}: {sample['action_reprs'][jj]}")
                            print(f"\n### Diffs of HTMLs:")
                            for d in diffs:
                                print(d)
            ...
            if permutation_possible:
                permutation_detection['n_permutable'] += 1
                print(f"Permutation possible: {permutation_detection['n_permutable']} / {permutation_detection['n_total']}")
