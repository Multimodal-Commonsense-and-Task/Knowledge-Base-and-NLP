...
def finetuning(opt, model, optimizer, scheduler, tokenizer, step):
    ...
    train_dataset = finetuning_data.HNDataset(
    ...
    collator = finetuning_data.HNCollator(tokenizer, passage_maxlength=opt.chunk_length)
    ...
    while step < opt.total_steps:
        ...
        log = f"{step} / {opt.total_steps} (epoch {epoch})"
        for k, v in sorted(run_stats.average_stats.items()):
            log += f" | {k}: {v:.3f}"
            if tb_logger:
                tb_logger.add_scalar(k, v, step)
        ...
        utils.save(model, optimizer, scheduler, step, opt, f'{opt.output_dir}_total', f"step-{step}")
...
def main():
    ...
    model_class = inbatch.HNInBatchClass
    if opt.weight_path!=None and os.path.exists(opt.weight_path):
        #state_dict = torch.load(opt.weight_path)
        model, optimizer, scheduler, step = utils.load(model_class, opt.weight_path, opt)
        model = model.to(device) #model = model.to('cuda:{}'.format(opt.device))
    else:
        ...
        model = model_class(opt, retriever, tokenizer)
    ...
    if opt.do_eval:
        ...
        evaluate(opt, model, tokenizer, None, step)
