...
def get_f1_score(predictions, answers):
    f1_list = []
    for i in range(len(answers)):
        f1_list.append(f1s(prediction[i], answers[i]))
    return np.mean(f1_list)

def check_answer_topk(example, k, tokenizer) -> List[bool]:
    """Search through all the top docs to see if they have any of the answers."""
    answers = example['answers']
    ctxs = example['ctxs'][:k]
...
def ctxs_recall(data, k=5):
    tkn = SimpleTokenizer()
    recall_list = []
    for i in range(len(data)):
        recall_list.append(max(check_answer_topk(data[i], k, tkn)))
