...
@register_method('bm25')
class BM25:
    def __init__(self, args: argparse.Namespace, tag: str, callbacks : list[Callback] | None = None):
        self.args = args
        self.retriever = Retriever(model_name='bm25', dataset=args.dataset, k1=args.bm25_k1, b=args.bm25_b)
    ...
    def retrieve(self, query : Query, k : int = 10) -> list[RankedDocument]:
        ...
        ranks = [rank for rank in ranks if rank.document.did not in query.excluded_dids][:k]
