...
QUERIES=$top_dir/datasets/${dataset}-queries-train-sampled.tsv
INDEX_NAME=${dataset}-colbert
...
ROOT_DIR=/your/path/colbert_trained/${dataset}/${case}

CHECKPOINT=$(find "${ROOT_DIR}/MSMARCO-psg/train.py/" -name "colbert-*.dnn" -type f | tail -n1)
echo "Loading the checkpoint file >>> $CHECKPOINT"

OUTPUT_DIR=${ROOT_DIR}/output
INDEX_ROOT=${ROOT_DIR}/index
RANKING_DIR=${ROOT_DIR}/ranking-train-sampled
...
python -m colbert.beir_eval \
  ...
  --data_dir "${top_dir}/datasets/${dataset}" \
  --output_file "${RANKING_DIR}/metric.json"
