...
class MonoT5ReRanker(TransformerBase):
    def __init__(self,
        ...
        self.tokenizer = T5Tokenizer.from_pretrained(tok_model, cache_dir=cache_dir, device_map='auto')
        self.model_name = model
        self.model = T5ForConditionalGeneration.from_pretrained(model, cache_dir=cache_dir, device_map='auto')
...
def test(model_name_or_checkpoint_path, cache_dir, batch_size, save_predictions_fn, test_filename_path, qrle_filename, save_evaluation_results):
    ...
    dir_path = os.path.dirname(save_predictions_fn)
    ...
    dir_path = os.path.dirname(save_evaluation_results)
    os.makedirs(dir_path, exist_ok=True)
    with open(save_evaluation_results, 'w') as f:
        json.dump(res, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_evaluation_results', required=True, type=str,
    ...
    test(args.model_name_or_checkpoint_path, args.cache_dir, args.batch_size, args.save_predictions_fn, args.test_filename_path, args.qrle_filename, args.save_evaluation_results)
