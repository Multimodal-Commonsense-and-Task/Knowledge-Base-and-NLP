...
if [ $iter != 0 ]; then
    ...
    checkpoint_dir=$(ls -d cocondenser_trained/${dataset}/${prev_case}_contriever_4_seed42_tevatron | sort -V | tail -n 1)
    ...
    $SCRIPT_DIR/scripts/run_epochs-cocondenser.sh $dataset "${case}_contriever_${num_pos_to_neg}" 1 $save_step 32 $run_name $lr $last_checkpoint
else
    ...
    $SCRIPT_DIR/scripts/qgen.sh $dataset $case
    $SCRIPT_DIR/scripts/hard_negative.sh $dataset $case false $num_pos_to_neg
    $SCRIPT_DIR/scripts/run_epochs-cocondenser.sh $dataset "${case}_contriever_${num_pos_to_neg}" 1 $save_step 32 $run_name $lr
fi