...
def main_run(dataset_name, fields, save_bm25_results_filepath, save_testdata_filepath, save_qrels_filepath, topk=100):
    ...
    # topics_filename = f"beir-v1.0.0-{dataset_name}-dev"
    # qrels_filename = f'beir-v1.0.0-{dataset_name}-dev'
    searcher = LuceneSearcher.from_prebuilt_index(lexical_index_name)


    # topics_dict = search.get_topics_with_reader("io.anserini.search.topicreader.TsvIntTopicReader", f"datasets/{dataset_name}/queries-train.tsv")
    topics_dict = search.get_topics_with_reader("io.anserini.search.topicreader.TsvStringTopicReader", f"datasets/{dataset_name}/queries-train.tsv")
    qrels = search.get_qrels_from_file(f"datasets/{dataset_name}/qrels/train.txt")
    qrels_formatted = {}
    ...
    python_cmd = f"python -m pyserini.search.lucene \
        ...
        --topics datasets/{dataset_name}/queries-train.tsv \
