...
from colbert.utils.utils import print_message, pickle_dump, pickle_load
...
def train(args):
    ...
    colbert = ColBERT.from_pretrained('bert-base-uncased',
                                      ...
                                      qidf=pickle_load(args.qidf) if args.qidf is not None else None,
    ...
    #optimizer = AdamW(filter(lambda p: p.requires_grad, colbert.lexical_linear.parameters()), lr=args.lr, eps=1e-8)
    optimizer.zero_grad()
    ...
    for batch_idx, BatchSteps in zip(range(start_batch_idx, args.maxsteps), reader):
        ...
        for queries, passages in BatchSteps:
            with amp.context():
                ...
                if args.regularizer == "istar":
                    scores, iso_loss = scores
                elif args.regularizer == "sensim":
                    scores, sensim_loss = scores
                elif args.regularizer == "cosreg":
                    scores, cosreg_loss = scores

                scores = scores.view(2, -1).permute(1, 0)
                loss = criterion(scores, labels[:scores.size(0)])

                if args.regularizer == "istar":
                    iso_loss = args.reg_lambda * iso_loss
                    print(f"loss: {loss}, iso_loss: {iso_loss}")
                    loss += iso_loss
                elif args.regularizer == "sensim":
                    sensim_loss = args.reg_lambda * sensim_loss
                    print(f"loss: {loss}, sensim: {sensim_loss}")
                    loss += sensim_loss
                elif args.regularizer == "cosreg":
                    cosreg_loss = args.reg_lambda * cosreg_loss
                    print(f"loss: {loss}, cosreg: {cosreg_loss}")
                    loss += cosreg_loss
