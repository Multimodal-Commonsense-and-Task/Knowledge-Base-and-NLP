...
def get_parts(directory):
    ...
    lexical_parts_paths = [os.path.join(directory, '{}.lexical'.format(filename)) for filename in parts]
    lexical_samples_paths = [os.path.join(directory, '{}.lexical_sample'.format(filename)) for filename in parts]

    return parts, parts_paths, samples_paths, lexical_parts_paths, lexical_samples_paths


def load_doclens(directory, flatten=True):
    parts, _, _, _, _ = get_parts(directory)
