...
def index_faiss(args):
    ...
    parts, parts_paths, samples_paths, lexical_parts_paths, lexical_samples_paths = get_parts(args.index_path)
    ...
    for slice_idx, part_offset in enumerate(range(0, len(parts), num_parts_per_slice)):
        ...
        print_message(f"{output_path} Done indexing!")
        ...
        # lexical index
        if os.path.isfile(f"{lexical_parts_paths[0]}"):
            slice_lexical_parts_paths = lexical_parts_paths[part_offset:part_endpos]
            slice_lexical_samples_paths = lexical_samples_paths[part_offset:part_endpos]

            lexical_faiss_index_name = "lexical_" + faiss_index_name
            lexical_output_path = os.path.join(args.index_path, lexical_faiss_index_name)  
            print_message(f"#> Will write to {lexical_output_path}.")
            lexical_index = prepare_faiss_index(slice_lexical_samples_paths, args.partitions, args.sample)

            thread = threading.Thread(target=_loader_thread, args=(slice_lexical_parts_paths,))
            thread.start()
            print_message("#> Indexing the vectors...")

            for filenames in grouper(slice_lexical_parts_paths, SPAN, fillvalue=None):
                print_message("#> Loading", filenames, "(from queue)...")
                sub_collection = loaded_parts.get()
                print_message("#> Processing a sub_collection with shape", sub_collection.shape)
                lexical_index.add(sub_collection)

            print_message(f"{lexical_output_path} Done indexing!")
            lexical_index.save(lexical_output_path)
            print_message(f"\n\nDone! All complete (for slice #{slice_idx+1} of {args.slices})!")
            thread.join()