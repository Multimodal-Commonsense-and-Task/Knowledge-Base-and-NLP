...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_step", default=200_000, required=True, type=int)
    ...
    if all_dataset:
        dataset_list = ["touche", "signal1m", "scifact", "climate-fever", "quora", "trec-covid", "bioasq", "fiqa", "trec-news", "nfcorpus", "nq", \
                        "robust04", "arguana", "dbpedia-entity", "scidocs", "cqadupstack/android", "cqadupstack/english", \
                        ...
                        "cqadupstack/webmasters", "cqadupstack/wordpress", "hotpotqa", "fever"]
    else:
    ...
    #checkpoint = f"/data/colbert-prf/experiments/colbertv2/colbertv2.0/pytorch_model.bin"
    ...
    for idx, dataset in tqdm(enumerate(dataset_list)):
        ...
        for i, docs_batch in enumerate(docs_batches):
            ...
            for pid, d, d_ids in zip(pids_batch, D, D_ids):
                pid_to_embs[pid] = d.to(torch.float32)
        ...
        if post_processing == "whitening":
            ...
            print("Glow Start!")
            glow_model_path = f"/data/colbert-prf/index/{index_path}/{dataset}/glow_model{bm25_filename}.pt"
            ...
            print("NICE Start!")
            nice_model_path = f"/data/colbert-prf/index/{index_path}/{dataset}/nice_model{bm25_filename}.pt"
            ...
            for batch in pids_batches:
                ...
                if dataset == "msmarco":
                    ...
                    for score, d_len in zip(scores, valid_pid_toks):
                        score_dist.append(score[:,:d_len].reshape(-1).cpu())
        ...
        if dataset == "msmarco":
            ...
            print(f"score shape: {score_dist.shape}, Sen-Sim: {torch.mean(score_dist.to(torch.float32))}")
            pickle_dump(score_dist, f"{save_path}/score_dist{bm25_filename}.pickle")

        ndcg = evaluation(hypo_colbert[dataset], qrels[dataset])
        print(f"{model_path} {dataset} post_processing: {post_processing} ndcg@10: {ndcg}")
