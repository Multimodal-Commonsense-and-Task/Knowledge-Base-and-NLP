...
class re_ranker_mmap:
    def __init__(self, index_path, args, inference, is_ours, verbose = False, memtype='mmap'):
        self.args = args
        ...
        #self.part_mmap : List[file_part_mmap], self.lexical_part_mmap : List[file_part_mmap] = re_ranker_mmap._load_parts(index_path, self.part_doclens, self.dim, memtype)
        self.part_mmap, self.lexical_part_mmap = re_ranker_mmap._load_parts(index_path, self.part_doclens, self.dim, memtype, is_ours=is_ours)
    ...
    @staticmethod
    def _load_parts(index_path, part_doclens, dim, memtype="mmap", is_ours=False):
        # Every pt file is loaded and managed independently, with local pids
        _, all_parts_paths, _, lexical_all_parts_paths, _= get_parts(index_path)
        
        if memtype == "mmap":
            #all_parts_paths = [ file.replace(".pt", ".store") for file in all_parts_paths ]
            all_parts_paths = [ file for file in all_parts_paths ]
            mmaps = [file_part_mmap(path, doclens, dim) for path, doclens in zip(all_parts_paths, part_doclens)]

            if is_ours:
                #lexical_all_parts_paths = [ file.replace(".pt", ".store") for file in lexical_all_parts_paths ]
                lexical_all_parts_paths = [ file for file in lexical_all_parts_paths ]
                lexical_mmaps = [file_part_mmap(path, doclens, dim) for path, doclens in zip(lexical_all_parts_paths, part_doclens)]
            else:
                lexical_mmaps = mmaps
            ...
            if is_ours:
                lexical_mmaps = [file_part_mem(path, doclens, dim) for path, doclens in tqdm(zip(lexical_all_parts_paths, part_doclens), total=len(lexical_all_parts_paths), desc="Loading index shards to memory", unit="shard")]
            else:
                lexical_mmaps = mmaps
        else:
            assert False, "Unknown memtype %s" % memtype
        return mmaps, lexical_mmaps
    ...
    def get_embedding_copy(self, pid, target, index, lexical=False):
        # In which pt file we need to look the given pid
        part_id = np.searchsorted(self.part_pid_end_offsets, pid)
        # calculate the pid local to the correct pt file
        local_pid = pid - self.part_pid_begin_offsets[part_id]
        # identify the tensor we look for
        if lexical:
            disk_tensor = self.lexical_part_mmap[part_id].get_embedding(local_pid)
        else:
            disk_tensor = self.part_mmap[part_id].get_embedding(local_pid)
        doclen = disk_tensor.shape[0]
        # only here is there a memory copy from the memory mapped file 
    ...
    def our_rerank_with_embeddings(self, qid, query, qembs, lexical_qembs, pids, passage, weightsQ=None, gpu=True):
        """
        ...
        colbert = self.args.colbert
        inference = self.inference
        batched_dtoks, dtoks_mask = inference.doc_tokenizer.tensorize(list(passage))
        qtoks, qtoks_mask = inference.query_tokenizer.tensorize([query])
        assert qtoks.shape[0] == 1, f"{qtoks.shape}, {qembs.shape}"
        qtoks_mask = qtoks_mask.squeeze()
        query_len = torch.sum(qtoks_mask).item()
        qtoks = qtoks.squeeze()
        qtoks = qtoks[:query_len]
        ...
        lexical_Q = torch.unsqueeze(lexical_qembs, 0)
        if gpu:
            Q = Q.cuda()
            lexical_Q = lexical_Q.cuda()
        ...
        D_ = torch.zeros(len(pids), self.doc_maxlen, self.dim)
        lexical_D_ = torch.zeros(len(pids), self.doc_maxlen, self.dim)
        for offset, pid in enumerate(pid_iter):
            self.get_embedding_copy(pid, D_, offset)
            self.get_embedding_copy(pid, lexical_D_, offset, lexical=True)
            
        if gpu:
            D_ = D_.cuda()
            lexical_D_ = lexical_D_.cuda()

        #maxscoreQ = weightsQ * (Q @ D_.permute(0, 2, 1)).max(2).values.cpu()
        #lexical_maxscoreQ = weightsQ * (lexical_Q @ lexical_D_.permute(0, 2, 1)).max(2).values.cpu()

        maxscoreQ, argmaxscoreQ = (Q @ D_.permute(0, 2, 1)).max(2)
        lexical_maxscoreQ, lexical_argmaxscoreQ = (lexical_Q @ lexical_D_.permute(0, 2, 1)).max(2)

        maxscoreQ = weightsQ * maxscoreQ.cpu()
        lexical_maxscoreQ = weightsQ * lexical_maxscoreQ.cpu()
        argmaxscoreQ = argmaxscoreQ.cpu()
        lexical_argmaxscoreQ = lexical_argmaxscoreQ.cpu()


        cnt = []
        assert len(pids) == len(batched_dtoks) == len(argmaxscoreQ) == len(lexical_argmaxscoreQ), f"{len(pids)}, {len(batched_dtoks)}, {len(argmaxscoreQ)}, {len(lexical_argmaxscoreQ)}"

        for pid, dtoks, argmaxes, lexical_argmaxes in zip(pids, batched_dtoks, argmaxscoreQ, lexical_argmaxscoreQ):
            for qtok, argmax, lexical_argmax in zip(qtoks, argmaxes, lexical_argmaxes):
                if qtok.item() not in self.idf:
                    continue
                if argmax.item() < len(dtoks) and qtok.item() == dtoks[argmax.item()]:
                    cnt.append(pid)

        em_rate = (len(cnt) / query_len) / len(batched_dtoks)
        return maxscoreQ.tolist(), lexical_maxscoreQ.tolist(), em_rate
        """

    def our_rerank_with_embeddings_batched(self, qid, query, qembs, lexical_qembs, pids, passage, weightsQ=None, gpu=True, batch_size=1000):
        import more_itertools
        if len(pids) < batch_size:
            #return self.our_rerank_with_embeddings(qembs, lexical_qembs, pids, weightsQ, gpu)
            scores, lexical_scores, em_rate = self.our_rerank_with_embeddings(qid, query, qembs, lexical_qembs, pids, passage, weightsQ, gpu)
            return scores, lexical_scores, em_rate

        #allscores=[]
        scores, lexical_scores, em_list, lx_em_list, cb_em_list = [], [], [], [], []
        #for group in more_itertools.chunked(pids, batch_size):
        for group, passage_group in zip(more_itertools.chunked(pids, batch_size), more_itertools.chunked(passage, batch_size)):
            #batch_scores = self.our_rerank_with_embeddings(qembs, lexical_qembs, group, weightsQ, gpu)
            #allscores.extend(batch_scores)
            maxscoreQ, lexical_maxscoreQ, em, lx_em, cb_em = self.our_rerank_with_embeddings(qid, query, qembs, lexical_qembs, group, passage_group, weightsQ, gpu)
            scores.extend(maxscoreQ)
            lexical_scores.extend(lexical_maxscoreQ)
            em_list.append(em)
            lx_em_list.append(lx_em)
            cb_em_list.append(cb_em)
            assert False

        return scores, lexical_scores, em_list, lx_em_list, cb_em_list


class ColBERTModelOnlyFactory():

    def __init__(self, 
            colbert_model : Union[str, Tuple[colbert.modeling.colbert.ColBERT, dict]], gpu=True, mask_punctuation=False, dim=128, qidf=None):
        args = Object()
...
class ColBERTFactory(ColBERTModelOnlyFactory):
    ...
    def __init__(self, 
            ...
            memtype = "mem",
            faisstype= "mem",
            qid_to_candidate_pid = None,
            cbpid_to_doc = None,
            is_ours=False,
            qidf=None,
            **kwargs):
        
        super().__init__(colbert_model, qidf=qidf, **kwargs)

        #esci_path = "../esci-data/shopping_queries_dataset"
        #self.qid_to_candidate_pid = pickle_load(os.path.join(esci_path, "qid_to_candidate_pid.pickle"))
        #self.query_to_qid = pickle_load(os.path.join(esci_path, "query_to_qid.pickle"))
        self.qid_to_candidate_pid = qid_to_candidate_pid # not used, for reranking
        self.cbpid_to_doc = cbpid_to_doc
        ...
        if index_root is None or index_name is None:
            ...
            docnos_file = os.path.join(self.index_path, "cbpid_to_id.pickle")
            if os.path.exists(docnos_file):
                with pt.io.autoopen(docnos_file, "rb") as f:
                    self.docid2docno = pickle.load(f)
                    self.docid2docno = list(self.docid2docno.keys())
                ...
                print(f"self.numdocs: {self.numdocs}")
            else:
    ...
    def _faiss_index(self):
        ...
        self.args.partitions = 100

        if self.is_ours:
            faiss_index_path = get_faiss_index_name(self.args)
            lexcial_faiss_index_path = "lexical_" + faiss_index_path
            faiss_index_path = os.path.join(self.index_path, faiss_index_path)
            lexcial_faiss_index_path = os.path.join(self.index_path, lexcial_faiss_index_path)
            ...
            self.lexical_faiss_index = FaissIndex(self.index_path, lexcial_faiss_index_path, self.args.nprobe, self.args.part_range, mmap=self.faisstype == 'mmap')
            ...
            if self.faiss_index_on_gpu:
                ...
                self.lexical_faiss_index.faiss_index = faiss.index_cpu_to_all_gpus(self.lexical_faiss_index.faiss_index)
            return self.faiss_index, self.lexical_faiss_index
    ...
    def set_retrieve(self, batch=False, query_encoded=False, faiss_depth=100, verbose=True, docnos=False) -> pt.Transformer:
        """
        ...
        assert not batch
        if self.is_ours:
            faiss_index, lexical_faiss_index = self._faiss_index()
        else:
            faiss_index = self._faiss_index()
        ...
        def _single_retrieve(queries_df):
            ...
            for row in iter:
                ...
                with torch.no_grad():
                    if self.is_ours:
                        Q, lexical_Q, ids, masks = self.args.inference.queryFromText([query], bsize=512, with_ids=True)
                        Q_f = Q[0:1, :, :]
                        lexical_Q_f = lexical_Q[0:1, :, :]
                    else:
                        Q, ids, masks = self.args.inference.queryFromText([query], bsize=512, with_ids=True)
                        Q_f = Q[0:1, :, :]

                if self.qid_to_candidate_pid is None:
                    colbert_all_pids = faiss_index.retrieve(faiss_depth, Q_f, verbose=False)

                    if self.is_ours:
                        lexical_all_pids = lexical_faiss_index.retrieve(faiss_depth, lexical_Q_f, verbose=False)
                        all_pids = [list(set(colbert_all_pids[0] + lexical_all_pids[0]))]
                    else:
                        lexical_all_pids = [[]]
                        all_pids = [list(set(colbert_all_pids[0]))]
                        lexical_all_pids[0] = set(lexical_all_pids[0])
                    colbert_all_pids[0] = set(colbert_all_pids[0])
                    
                else:
                    all_pids = [self.qid_to_candidate_pid[qid]]

                #Q_cpu = Q[0, :, :].cpu()
                #lexical_Q_cpu = lexical_Q[0, :, :].cpu()

                for passage_ids in all_pids:
                    if verbose:
                        print("qid %s retrieved docs %d" % (qid, len(passage_ids)))
                    for pid in passage_ids:
                        if self.cbpid_to_doc is None:
                            rtr.append([qid, query, pid, ids[0], (pid in lexical_all_pids[0]), (pid in colbert_all_pids[0])])
                        else:
                            passage = self.cbpid_to_doc[pid]
                            rtr.append([qid, query, pid, passage, ids[0], (pid in lexical_all_pids[0]), (pid in colbert_all_pids[0])])
                        
            #build the DF to return for this query
            if self.cbpid_to_doc is None:
                rtrDf = pd.DataFrame(rtr, columns=["qid","query",'docid','query_toks','islx','iscb'])
            else:
                rtrDf = pd.DataFrame(rtr, columns=["qid","query",'docid','passage','query_toks','islx','iscb'])
            if docnos:
        ...
        def _single_retrieve_qembs(queries_df):
            ...
            for row in iter:
                ...
                lexical_embs = row.query_embs
                lexical_Q_f = torch.unsqueeze(lexical_embs, 0)
                
                if self.qid_to_candidate_pid is None:
                    all_pids = faiss_index.retrieve(faiss_depth, Q_f, verbose=verbose)
                    lexical_all_pids = lexical_faiss_index.retrieve(faiss_depth, lexical_Q_f, verbose=verbose)
                    all_pids[0].extend(lexical_all_pids[0])
                    all_pids[0] = list(set(all_pids[0]))
                else:
                    all_pids = [self.qid_to_candidate_pid[qid]]
                ...
                for passage_ids in all_pids:
                    ...
                    for pid in passage_ids:
                        if query_weights:
                            if self.cbpid_to_doc is None:
                                rtr.append([qid, row.query, pid, row.query_toks, row.query_embs, row.lexical_query_embs, row.query_weights])
                            else:
                                passage = self.cbpid_to_doc[pid]
                                rtr.append([qid, row.query, pid, passage, row.query_toks, row.query_embs, row.lexical_query_embs, row.query_weights])
                        else:
                            if self.cbpid_to_doc is None:
                                rtr.append([qid, row.query, pid, row.query_toks, row.query_embs, row.lexical_query_embs])
                            else:
                                passage = self.cbpid_to_doc[pid]
                                rtr.append([qid, row.query, pid, passage, row.query_toks, row.query_embs, row.lexical_query_embs])

            
            #build the DF to return for this query
            if self.cbpid_to_doc is None:
                cols = ["qid","query",'docid','query_toks','query_embs','lexical_query_embs']
            else:
                cols = ["qid","query",'docid','passage','query_toks','query_embs','lexical_query_embs']
            if query_weights:
    ...
    def index_scorer(self, query_encoded=False, add_ranks=False, add_docnos=True, batch_size=10000) -> pt.Transformer:
        ...
        def rrm_scorer_query_embs(qid_group):
            ...
            qid_group.sort_values("docid", inplace=True)
            docids = qid_group["docid"].values
            passage = qid_group["passage"].values if "passage" in qid_group.columns else None
            weights = None
            if "query_weights" in qid_group.columns:
                weights = qid_group.iloc[0].query_weights
            if batch_size > 0:
                #scores = rrm.our_rerank_with_embeddings_batched(qid_group.iloc[0]["query_embs"], qid_group.iloc[0]["lexical_query_embs"], docids, weights, batch_size=batch_size, gpu=self.gpu)
                cb_scores, lx_scores, em_rate = rrm.our_rerank_with_embeddings_batched(qid_group.iloc[0]["qid"], qid_group.iloc[0]["query"], qid_group.iloc[0]["query_embs"], \
                                                                                                qid_group.iloc[0]["lexical_query_embs"], docids, passage, weights, batch_size=batch_size, gpu=self.gpu)
            else:
                cb_scores, lx_scores, em_rate = rrm.our_rerank_with_embeddings(qid_group.iloc[0]["qid"], qid_group.iloc[0]["query"], qid_group.iloc[0]["query_embs"], \
                                                                                        qid_group.iloc[0]["lexical_query_embs"], docids, passage, weights, gpu=self.gpu)
            qid_group["score"] = 0
            qid_group["cb_score"] = cb_scores
            qid_group["lx_score"] = lx_scores
            qid_group["em_rate"] = em_rate
    ...
    def first_stage(self) -> pt.Transformer:
        return self.set_retrieve()
