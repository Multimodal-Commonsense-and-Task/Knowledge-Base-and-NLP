...
class CollectionEncoder():
    def __init__(self, args, process_idx, num_processes):
        ...
        self.process_idx = max(0, self.args.rank)
        self.num_processes = self.args.nranks
        ...
        self.print_main(f"#> self.process_idx = {self.process_idx}")
        self.print_main(f"#> self.num_processes = {self.num_processes}")
        self.print_main(f"#> self.args.rank = {self.args.rank}")
        self.print_main(f"#> self.args.nranks = {self.args.nranks}")
...
class ColBERTIndexer(pt.Indexer):
    def __init__(self, checkpoint, index_root, index_name, chunksize, rank=-1, prepend_title=False, num_docs=None, ids=True, gpu=True, mask_punctuation=False, qidf=None):
        args = Object()
        args.qidf = qidf
    ...
    def index(self, iterator):
        ...
        if num_embeddings == 0:
            ...
            num_embeddings = sum(load_doclens(self.args.index_path))
            encoded = False
            print("#> encoded num_embeddings =", num_embeddings)
...
class CollectionEncoderIds(CollectionEncoder_Generator):
    def _encode_batch(self, batch_idx, batch):
        with torch.no_grad():
            # IsoBERT
            if self.args.qidf:
                embs, lexical_embs, ids = self.inference.docFromText(batch, bsize=self.args.bsize, keep_dims=False, with_ids=True)
                assert type(embs) is list
                assert len(embs) == len(batch)
                assert type(lexical_embs) is list
                assert len(lexical_embs) == len(batch)
                ...
                embs = torch.cat(embs)
                lexical_embs = torch.cat(lexical_embs)
                ids = torch.cat(ids)
                return embs, lexical_embs, local_doclens, ids
    ...
    def _save_batch(self, batch_idx, embs, offset, doclens, ids, lexical_embs=None):
        start_time = time.time()
        ...
        if self.args.qidf:
            output_lexical_path = os.path.join(self.args.index_path, "{}.lexical".format(batch_idx))
            output_lexical_sample_path = os.path.join(self.args.index_path, "{}.lexical_sample".format(batch_idx))
        ...
        if self.args.qidf:
            self.indexmgr.save(lexical_embs, output_lexical_path)
            self.indexmgr.save(lexical_embs[torch.randint(0, high=embs.size(0), size=(embs.size(0) // 20,))], output_lexical_sample_path)
    ...
    def encode(self):
        ...
        for batch_idx, (offset, lines, owner) in enumerate(self._batch_passages(self.iterator)):
            ...
            if batch_idx % 100 == 0:
                print(f"self.process_idx: {self.process_idx}, batch_idx: {batch_idx}")

            t1 = time.time()
            batch = self._preprocess_batch(offset, lines)

            if self.args.qidf:
                embs, lexical_embs, doclens, ids = self._encode_batch(batch_idx, batch)
                t2 = time.time()
                self.saver_queue.put((batch_idx, embs, offset, doclens, ids, lexical_embs))
            else:
            ...
            self.print(f'#> Completed batch #{batch_idx} (starting at passage #{offset}) \t\t'
                    ...
                    f'{this_encoding_throughput} (this encoding), ',
                    f'{this_saving_throughput} (this saving)')
            print(f"batch_idx: {batch_idx} / offset: {offset} / owner: {owner}")
