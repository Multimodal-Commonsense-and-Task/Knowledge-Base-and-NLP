...
def is_valid_format(output, question, prompt_type = "batch", num_groups = 1):
    ...
    This example assumes that the output should be non-empty and start with the answer.
    Modify the validation logic based on your specific requirements.
    """
    # Extract the relevant part of the output after the question
    extracted_output = output.split(f"\nQuestion: {question}")[-1].strip()
    extracted_output = extracted_output.lower()
    # Add your specific validation logic here
    if prompt_type in ["stepback"]:
        valid = ("step 1" in extracted_output or "step1" in extracted_output) \
              and ("step 2" in extracted_output or "step2" in extracted_output) \
                and "final answer" in extracted_output
    elif prompt_type in ["batch"]:
        # valid = ("step 1" in extracted_output or "step1" in extracted_output) \
        #       and ("step 2" in extracted_output or "step2" in extracted_output)
        valid2 = [f"a[{i+1}]" in extracted_output for i in range(num_groups)]
        # valid = valid and all(valid2)
        valid = all(valid2)
    else:
        valid = len(extracted_output) > 0
    return valid
...
parser.add_argument("--prompt-type", type=str, default="sp", choices=["sp", "icl", "stepback", "batch", "stepback1", "stepback2"], help="Prompt type")
...
parser.add_argument("--output-path", type=str, default="output/sampled300/output.json", help="Output path")
...
max_new_tokens = 16 if (not args.do_refine and args.prompt_type in ['icl', 'sp']) else 512
...
if "easy" in args.data_path:
    args.output_path = os.path.join(os.path.dirname(args.output_path), "easy", os.path.basename(args.output_path))
if not args.do_refine:
    # [basename]/[prompt_type]/[model_name]/[datetime yy mm dd hh]-[num_samples].json
    args.output_path = os.path.join(os.path.dirname(args.output_path), args.prompt_type, args.model_path.rsplit('/', 1)[1], f"{datetime.datetime.now().strftime('%y%m%d%h')}-{args.num_samples}.json")
...
def initial_answer(args):
    if args.prompt_type == "icl":
        icl_file  = "prompt/prompt_ICL_sp.txt"
        icl_examples = open(icl_file, "r").read()
        template = "Answer the questions with a brief phrase, no explanations.\n\n"
        template += f"""Example:\n\n{icl_examples}\n\nTest:\n\n"""
        template += """Story: {context}\n\nQuestion: {question}\n\nAnswer: """    
    elif args.prompt_type == "sp":
        template = "Answer the questions with a brief phrase, no explanations.\n\n"
        template = """Story: {context}\n\nQuestion: {question}\n\nAnswer: """
    elif args.prompt_type == "stepback":
        icl_file  = "prompt/prompt_ICL_stepback.txt"
        icl_examples = open(icl_file, "r").read() 
        template = f"""{icl_examples}\n\n\n\nTest:\n\n\n"""
        template += """Story: {context}\n\n\nQuestion: {question}\n\n\nAnswer: """
    elif args.prompt_type == "stepback1":
        icl_file  = "prompt/prompt_stepback_question1.txt"
        icl_examples = open(icl_file, "r").read() 
        template = f"""{icl_examples}\n\n\n\nTest:\n\n\n"""
        template += """Context: {context}\n\n\nQuestion: {question}\n\n\nAnswer: """
    elif args.prompt_type == "stepback2":
        icl_file  = "prompt/prompt_stepback_question2.txt"
        icl_examples = open(icl_file, "r").read() 
        template = f"""{icl_examples}\n\n\n\nTest:\n\n\n"""
        template += """Context: {context}\n\n\nQuestion: {question}\n\n\nAnswer: """
    elif args.prompt_type == "batch":
        template = open("prompt/prompt_batch.txt", "r").read()
        template += """\n\n\nTest:\n\nContext: {context}\n\n{question}\n\nAnswer: """    

    prompt = PromptTemplate(template=template, input_variables=["question", "context"])

    chain = prompt | hf
    generator = RetryGeneration(chain, max_retries=3)

    if args.prompt_type in ["batch"]:
    
        last_idx = None
        groups = []
        for input in inputs:
            idx = input["idx"].split("#")[0]
            if idx != last_idx:
                groups.append([])
            groups[-1].append(input)
            last_idx = idx
        
        with open(args.output_path, "w") as f:
            for group in tqdm(groups):
                context = group[0]["context"]
                # predictions = [input["prediction"] for input in group] # number each answer.
                questions = []
                for i, input in enumerate(group):
                    questions.append(f"Question {i+1}: {input['question']}")
                questions = "\n\n".join(questions)
                # output = chain.invoke({"context": context, "question": questions})
                output = generator(context, questions, args.prompt_type, num_groups=len(group))
                output = output.split(questions)[-1]
                output = output.strip()
                json_output = {"idx": group[0]["idx"], "answer": output}
...
def consistent_answer(args):
    template = open("prompt/prompt_consistency.txt", "r").read()
    template += """\n\n\nTest:\n\nContext: {context}\n\n{question}\n\nAnswer:\n"""
    prompt = PromptTemplate(template=template, 
                            input_variables=["context", "question"])
    chain = prompt | hf
    generator = RetryGeneration(chain, max_retries=3)
    # group answers from initial answers. check if there is any temporal inconsistency. fix the answers.
    with open(args.output_path, "r") as f:
        outputs = [json.loads(line) for line in f]
    # merge with input
    for i in range(len(inputs)):
        assert inputs[i]["idx"] == outputs[i]["idx"]
        inputs[i]["prediction"] = outputs[i]["answer"]
    ...
    refined_output_path = args.output_path.replace(".json", "_refined.json")
    with open(refined_output_path, "w") as f:
        ...
        for group in tqdm(groups):
            ...
            predictions = []
            for i, input in enumerate(group):
                if "\n\nStep 2:" in input["prediction"]:
                    input["prediction"] = input["prediction"].split("\n\nStep 2:")[1]
                input["prediction"] = input["prediction"].lstrip("Answer:").strip()
                predictions.append(f"Question {i+1}: {input['question']}\nPrediction {i+1}: {input['prediction']}")
            predictions = "\n\n".join(predictions)
            output = generator(context, predictions, args.prompt_type, num_groups=len(group))
            # output = chain.invoke({"context": context, "prediction": predictions})
            output = output.split(predictions)[-1]
            output = output.strip()
            json_output = {"idx": group[0]["idx"], "answer": output}
            f.write(json.dumps(json_output) + "\n")
