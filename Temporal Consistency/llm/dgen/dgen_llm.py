...
def is_valid_format(output, paragraphs, prompt_type = "batch", num_groups = 1, input_paragraphs = None):
    """
    ...
    output = output.split(f"{paragraphs}\n\nSelected paragraph title: ")[-1].strip().lower()
    # if any([paragraph for paragraph in input_paragraphs if paragraph["title"].lower() in output]):
    #     return True
    if output in [paragraph["title"].lower() for paragraph in input_paragraphs]:
        return True
    return False
...
class RetryGeneration():
    ...
    def __call__(self, paragraphs, question, prompt_type, num_groups = 1, input_paragraphs = None):
        for i in range(self.max_retries):
            output = self.chain.invoke({"paragraphs": paragraphs, "question": question})
            if is_valid_format(output, paragraphs, prompt_type, num_groups, input_paragraphs):
                return output, True
        return output, False
...
parser.add_argument("--prompt-type", type=str, default="sp", choices=["sp", "stepback", "batch"], help="Prompt type")
...
parser.add_argument("--data-path", type=str, default="dataset/test.hard.concat.json", help="Data path")
parser.add_argument("--output-path", type=str, default="output/dgen/dgen.json", help="Output path")
...
max_new_tokens = 2048 if args.prompt_type == "sp" else 48
...
if "easy" in args.data_path:
    args.output_path = os.path.join(args.output_path.split("/")[0], "dgen", "easy", args.output_path.split("/")[-1])
if not args.do_refine:
    # [basename]/[prompt_type]/[model_name]/[datetime yy mm dd hh]-[num_samples].json
    args.output_path = os.path.join(args.output_path.split("/")[0], "dgen", args.prompt_type, args.model_path.rsplit('/', 1)[1], f"{datetime.datetime.now().strftime('%y%m%d%h')}-{args.num_samples}.json")
...
def initial_answer(args):
    if args.prompt_type == "sp":
        # select one most relevant paragraph
        template = """Select the most relevant paragraph to answer the following question. The output should only contain the title of the selected paragraph. """
        template += """\n\n\nQuestion: {question}\n\nParagraphs: {paragraphs}\n\nSelected paragraph title: """
        prompt = PromptTemplate(template=template, input_variables=["question", "paragraphs"])
        chain = prompt | hf
        generator = RetryGeneration(chain, max_retries=5)
        with open(args.output_path, "w") as f:
            for input in tqdm(inputs):
                paragraphs = input['paragraphs'][1:]
                question = input['question']
                if len(paragraphs) == 0:
                    new_input = input
                    f.write(json.dumps(new_input) + "\n")
                output = generator(paragraphs, question, args.prompt_type, input_paragraphs = input['paragraphs'])
                # get the answer from the output
                output = output.split(f"{paragraphs}\n\nSelected paragraph title: ")[-1].strip()
                json_output = {"idx": input["idx"], "paragraph_title": output}
