...
parser.add_argument("--output-path", type=str, default="output/qgen/qgen.json", help="Output path")
...
max_new_tokens = 512 if args.prompt_type == "sp" else 48
...
if "easy" in args.data_path:
    args.output_path = os.path.join(args.output_path.split("/")[0], "qgen", "easy", args.output_path.split("/")[-1])
if not args.do_refine:
    # [basename]/[prompt_type]/[model_name]/[datetime yy mm dd hh]-[num_samples].json
    args.output_path = os.path.join(args.output_path.split("/")[0], "qgen", args.prompt_type, args.model_path.rsplit('/', 1)[1], f"{datetime.datetime.now().strftime('%y%m%d%h')}-{args.num_samples}.json")
...
def initial_answer(args):
    if args.prompt_type == "sp":
        template = open("prompt/qgen/qgen.txt", "r").read()
        template += """\n\n\nTest:\n\nContext: {context}\n\nQuestion: {question}\n\nRelated questions: """
        ...
        template = open("prompt/qgen/stepback_q.txt", "r").read()
        template += """\n\n\nTest.\n\nTime-sensitive questions:\n{question}\n\nTime-agnostic question:\n"""
        ...
        with open(args.output_path, "w") as f:
            for group in tqdm(groups):
                ...
                questions = "\n".join(questions)
                output = chain.invoke({"question": questions})
                output = output.split(questions+"\n\nTime-agnostic question:\n")[-1]
                output = output.strip()
